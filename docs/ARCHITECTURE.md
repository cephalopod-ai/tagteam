...
| Control-plane contract | `internal/tagteam/control_contract.go`, `control_runtime.go`, `mcp_stdio.go`, `internal/cli/mcp.go` | Versioned launch/action types, bounded projections, durable approval/idempotency records, and a local MCP stdio transport with approved start. Resume and cancel remain gated. |
