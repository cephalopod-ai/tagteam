...
| Control-plane contract | `internal/tagteam/control_contract.go`, `mcp_stdio.go`, `internal/cli/mcp.go` | Versioned launch/action types plus bounded read-only projections and a local MCP stdio transport. Mutating controller operations remain gated. |
