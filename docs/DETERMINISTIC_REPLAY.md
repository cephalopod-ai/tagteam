...
Status: accepted; artifact contract version 2.
...
and fencing generation. Each record carries an integrity digest, and committed
results carry a digest of the representation that is actually persisted. The
unique sequence filename enforces `(run, sequence)`; reads validate a contiguous
stream from sequence one, and durable atomic replacement commits transitions.
...
There is no fabricated backfill. Runs without a valid version 2
`execution-snapshot.json` are
...
them. Version 1 snapshots and operation records are deliberately not upgraded:
their canonical bytes and records lack the version 2 integrity guarantees.
Never delete operation artifacts to unblock a run. For
...
| Sequence and exact replay | `prepareOperation`, `transitionOperation` | `TestExactCommittedReplayDoesNotInvokeAgain`, `TestUnresolvedMismatchAndCorruptionFailClosed`, `TestCommittedOperationIntegrityAndLifecycleFailClosed` |
...
Tagteam currently schedules adapter roles serially, so production panel
requirements are one member. The durable panel repository and concurrency tests
enforce all-or-nothing multi-member admission, but no user-facing workflow mode
currently requests a multi-member parallel panel. This is not a claim that an
unwired parallel execution mode exists.
