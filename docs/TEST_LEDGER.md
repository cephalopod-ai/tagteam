...
| Control-plane producer contract | `go test ./internal/tagteam/ -run 'Test(Control\|NormalizeControl)'` | pass | `control_contract_test.go` | Canonical repository/action identity, mode-role validation, path traversal rejection, bounded status, plan/findings pagination, deterministic digests, and read-only diagnostics | Mutating lifecycle and MCP transport remain gated |
| MCP stdio transport | `go test ./internal/tagteam/ ./internal/cli -run 'TestMCP'` | pass | `mcp_stdio_test.go`, `root_test.go` | MCP initialization, deterministic read-only tool listing, structured results/errors, and CLI wiring | Start/resume/cancel remain gated |
