...
| Control-plane producer contract | `go test ./internal/tagteam/ -run 'Test(Control\|NormalizeControl\|ControlRuntime)'` | pass | `control_contract_test.go`, `control_runtime_test.go` | Canonical repository/action identity, mode-role validation, path traversal rejection, bounded status, durable start IDs, approval binding, nonce replay rejection, idempotency, and persisted preflight failure | Resume/cancel remain gated; stale approval-ledger lock fails closed |
| MCP stdio transport | `go test ./internal/tagteam/ ./internal/cli -run 'TestMCP'` | pass | `mcp_stdio_test.go`, `root_test.go` | MCP initialization, deterministic tool listing, launch/start preparation, structured results/errors, approved start handles, and CLI wiring | Resume/cancel remain gated |
