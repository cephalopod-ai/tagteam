...
| Claude output hardening | `go test ./internal/tagteam/` (`adapters_part02_test.go`, 9 tests) | pass | `adapters_part02_test.go` | Claude envelope `is_error`/`error_*` surfacing, embedded-JSON candidate recovery (fenced blocks, decoy objects) across worker/review contracts | Real Claude prose variants are sampled, not exhaustive |
| Claude invocation serialization | `go test ./internal/tagteam/` (`invocation_lock_test.go`, 6 tests) | pass | `invocation_lock_test.go` | Cross-process PID lock acquire/release, live-holder wait, stale-lock reclaim, cancellation, degrade-to-unlocked timeout | Does not exercise two real tagteam processes contending |
