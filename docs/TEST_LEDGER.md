...
| Adapters and role policy | `go test ./internal/tagteam/` (`adapters_test.go`, `config_test.go`) | pass | `adapters_test.go`, `config_test.go` | argv construction, capabilities, schema wiring, per-role env/sandbox, Grok headless/model/effort/tools/permissions and structured parsing, Claude supervisor/adversary allowance, and Claude implementation/scout rejection | Live Grok authentication/model execution remains unexercised |
...
| Control-plane producer contract | `go test ./internal/tagteam/ -run 'Test(Control\|NormalizeControl\|ControlRuntime)'` | pass | `control_contract_test.go`, `control_runtime_test.go`, `control_resume_assessment_test.go` | Canonical repository/action identity, server-worktree binding, mode-role validation, path traversal rejection, bounded status, durable start IDs, approval binding, nonce replay rejection, idempotency, persisted preflight failure, and read-only resume assessment | Resume/cancel remain gated; stale approval-ledger lock fails closed |
| MCP stdio transport | `go test ./internal/tagteam/ ./internal/cli -run 'TestMCP'` | pass | `mcp_stdio_test.go`, `root_test.go` | MCP initialization, deterministic tool listing, launch/start/resume preparation, structured results/errors, approved start handles, and CLI wiring | Resume/cancel remain gated |
